...
async function health(chatId, env) {
  ...
  // ПАРАЛЛЕЛЬНО, а не по очереди: лимит waitUntil = 30 сек на весь запрос.
  ...
  const checks = await Promise.all(
    getAllProviders(env).map(async (provider) => {
      ...
      try {
        ...
        return { provider, ok, ms: Date.now() - started, error: attempt?.error };
      } catch (e) {
        return { provider, ok: false, ms: Date.now() - started, error: String(e) };
      }
    })
  );
  ...
  for (const c of checks) {
    lines.push(
      `${c.ok ? "✅" : "❌"} <b>${c.provider.id}</b> — ${c.ms} мс` +
        (c.ok ? "" : `\n   <code>${escapeHtml(String(c.error || "").slice(0, 150))}</code>`)
    );
  }
