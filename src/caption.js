  ...
  return [
    {
      ...
      content:
        ...
        "описания и примеров.\n\n" + 
        "Формат:\n" +
        "1. Поздоровайся своими словами ровно один раз. Можно «Доброе утро», можно " +
        "иначе — лишь бы звучало живо и по-разному каждый раз. " +
        "Не начинай каждое предложение заново с приветствия. " +
        "НЕ копируй примеры из этой инструкции дословно.\n" +
        "1а. Не пиши несколько вариантов подписи подряд. Нужен один цельный текст, " +
...
export function captionProblem(text) {
  ...
  // Не показываем оборванные ответы. Раньше cleanup добавлял «…», из-за чего
  // в чат уходили подписи, которые фактически заканчивались на полуслове.
  if (/…$/.test(out) || !/[.!?»)]$/.test(out)) return "незаконченное последнее предложение";
